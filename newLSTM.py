...
class A_LSTMCell_1(rnn.BasicLSTMCell):
    """
    MY LSTM proposed.
    """
    def __init__(self,
        ...
        Initialize the basic LSTM cell.
        """        
        super(A_LSTMCell_1,self).__init__(num_units,
               forget_bias=1.0,
               state_is_tuple=True,
               activation=None,
               reuse=None,
    ...
    def build(self, inputs_shape):
        ...
        self._v_bias = self.add_variable(
            'v_bias',
...
class deepLSTM(rnn.BasicLSTMCell):
    def __call__(self,inputs,state,scope=None):
        """Long short-term memory cell (LSTM)."""
        with tf.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
        # Parameters of gates are concatenated into one multiply for efficiency.
...
class NewAttention_Layer():
    ...
    def __call__(self,values,queries):
        """
        producing output with actual inputs.
        ...
        """
        #temp = tanh(Y X W + b) ->shape of result = (-1, self.o_size)
        temp=tf.layers.dense(tf.reshape(queries,[-1,self.h_size]),self.h_size,tf.tanh)
...
class NewAttention():
    def __init__(
        ...
        self.queryLSTM=rnn.BasicLSTMCell(hidden_unit_size)
